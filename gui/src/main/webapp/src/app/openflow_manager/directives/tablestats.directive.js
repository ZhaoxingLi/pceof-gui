              ...
              var getData = function() {
                  ...
                  if ( $scope.graphTableData.length && $scope.graphTableData[0]['stats-array'].length ){
                      ...
                      for( var prop in $scope.graphTableData[0]['stats-array'][0] ){
                          ...
                          var column = {
                            name: prop,
                            show: prop === 'BytesTransmitted' || count < 4 || columnToShow.indexOf(prop) !== -1 ? true : false,
                  ...
                  $scope.graphTableData.forEach(function(dev){
                    if ( dev['stats-array'] && dev['stats-array'].length ) {
                      dev['stats-array'].forEach(function(table){
                        try {
                          var topologyNode =  $scope.$parent.getNodeByDataPath(table.Device);

                          table.Device = topologyNode['node-id'];

                          var terminationPoint = $scope.$parent.getTpByPortId(topologyNode.tp, table.Id);

                          table.Id = terminationPoint['tp-id'];

                          $scope.$parent.getTpByDataPath(table.Device, table.Id);
                        }
                        catch(e) {
                          //console.log('Failed device: ', table);
                        }
